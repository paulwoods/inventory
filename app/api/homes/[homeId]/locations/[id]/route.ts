...
type Params = { params: { homeId: string; id: string } };
...
export async function GET(_req: Request, {params}: Params) {
    ...
    const loc = await getLocation(params.homeId, params.id);
    if (!loc) {
...
export async function PUT(req: Request, {params}: Params) {
    ...
    try {
        ...
        const updated = await updateLocation(params.homeId, params.id, {
            name: json.name,
...
export async function DELETE(_req: Request, {params}: Params) {
    ...
    const ok = await deleteLocation(params.homeId, params.id);
    if (!ok) {
        const body: ApiResponse<never> = {ok: false, error: 'Location not found.'};
        return NextResponse.json(body, {status: 404});
    }
