...
export default function LocationForm({homeId, initial, mode, onCancel, onSaved}: Props) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setSubmitting(true);
        try {
            const payload: LocationInput = {name: name.trim(), description: description.trim() || undefined};
            let res: Response;
            if (mode === 'create') {
                res = await fetch(`/api/homes/${homeId}/locations`, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify(payload)
                });
            } else {
                res = await fetch(`/api/homes/${homeId}/locations/${initial?.id}`, {
                    method: 'PUT',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify(payload)
                });
